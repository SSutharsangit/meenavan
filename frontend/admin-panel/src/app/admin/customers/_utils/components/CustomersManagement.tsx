...
export default function CustomersManagement() {
  ...
  const fetchData = async (page = 1) => {
    try {
      setLoading(true);
      const res = await apiGetAllCustomers(page, debouncedSearch || undefined);
      if (res.is_success) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try { const res = await apiDeleteCustomer(id); if (res.is_success) fetchData(pagination?.current_page || 1); } catch (e) { console.error(e); }
  };
