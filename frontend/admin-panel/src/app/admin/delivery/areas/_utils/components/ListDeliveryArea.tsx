...
export default function ListDeliveryArea({ data, loading, pagination, onEdit, onDelete, searchValue, onSearchChange, onPageChange }: Props) {
  const columns: Column<any>[] = [
    {
      key: "name_en", label: "Area Name",
      render: (row) => (
        <div>
    ...
    {
      key: "postal_codes", label: "Postal Codes",
      render: (row) => (
        <span className="text-slate-600 font-mono text-xs">
          {Array.isArray(row.postal_codes) ? row.postal_codes.join(', ') : (row.postal_codes || "—")}
        </span>
      ),
    },
    {
      key: "delivery_time", label: "Est. Time",
      render: (row) => <span className="text-slate-600 font-medium">{row.delivery_time_min} - {row.delivery_time_max} mins</span>,
    },
    {
